...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # self.contribution_weight = nn.Parameter(torch.tensor(1.0))  # Initialize the contribution_weight
    ...
    def forward(self, image, tokenized_prompts, pos_embedding=False):
        ...
        # text_features shape: [num_of_texts, 2, embed_dim]
        # new shape: [num_of_texts * 2, embed_dim]
        text_features_stacked = text_features.view(-1, text_features.size(-1))

        # Compute logits per image for each text feature
        # image_features shape: [batch_size, embed_dim]
        # logits shape: [batch_size, num_of_texts * 2]
        ...
        # # probs shape: [batch_size, num_of_texts, 2]
        # probs = logits.softmax(dim=-1)

        # Extract the first probability from each pair
        # pred shape: [batch_size, num_of_texts]
        pred_emb = logits

        return pred_emb # normal usage
        # return image_features # for vision encoder benchmarking
...
def build_model(state_dict: dict):
    ...
    # # Check if the state_dict contains 'contribution_weight'
    # if "contribution_weight" not in state_dict:
    #     state_dict["contribution_weight"] = torch.tensor(1.0)
