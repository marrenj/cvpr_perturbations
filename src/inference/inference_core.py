...
def compute_cka(X, Y, debiased=False):
    ...
    Args:
        ...
        debiased (bool): Use the unbiased HSIC estimator. Recommended for
...
def run_inference(config):
    ...
    if evaluation_type in ("neural", "behavioral"):
        ...
        annotations_df = pd.read_csv(config['annotations_file'], index_col=0)
        ...
        debiased = config.get('cka_debiased', False)
        cka_score = compute_cka(model_activations, spose_embeddings, debiased=debiased)
        score = cka_score
        ...
        logger.info(
            "CKA score (epoch %s, n=%d, debiased=%s): %.4f",
            epoch, n_images, debiased, cka_score,
        )
        results_json_path = (
            Path(config['inference_save_dir'])
