...
def shuffle_targets(targets, shuffle_seed=None):
    """
...
def train_model(model, train_loader, test_loader, inference_loader, device, optimizer, criterion, epochs, training_res_path, batch_size, csv_file, img_dir, early_stopping_patience=5, checkpoint_path='clip_hba_model_cv.pth', dora_parameters_path='./dora_params', model_embedding_path='./model_embeddings', model_rdm_path='./model_rdms', random_target_epoch=None, random_target_seed=42):
    ...
    headers = ['epoch', 'train_loss', 'test_loss', 'behavioral_rsa_rho', 'behavioral_rsa_p_value', 'used_shuffled_targets']
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (_, images, targets, _) in progress_bar:
            ...
            # Check if this is the epoch where we should use shuffled targets
            if random_target_epoch is not None and epoch == random_target_epoch - 1:  # -1 because epochs are 0-indexed
                ...
                print(f"Shuffle target seed: {random_target_seed}")
                targets = shuffle_targets(targets, random_target_seed)
            ...
            loss = criterion(predictions, targets)
            progress_bar.set_postfix({'loss': loss.item(), 'shuffled_targets': used_random_targets})
            loss.backward()
            optimizer.step()
        ...
        if used_random_targets:
            print(f"*** SHUFFLED TARGETS WERE USED IN THIS EPOCH ***")
...
def run_behavioral_traning(config):
    ...
    # Print shuffled target information if specified
    if config.get('random_target_epoch') is not None:
        print(f"*** SHUFFLED TARGET CONFIGURATION ***")
        print(f"Shuffled targets will be used in epoch: {config['random_target_epoch']}")
        print(f"Shuffle target seed: {config.get('random_target_seed', 42)}")
        print("*** END SHUFFLED TARGET CONFIGURATION ***\n")
