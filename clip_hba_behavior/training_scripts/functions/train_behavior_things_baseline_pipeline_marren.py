...
def train_model(model, train_loader, test_loader, inference_loader, difficulty_loader, device, optimizer, criterion, epochs, training_res_path, difficulty_criterion, batch_size, csv_file, img_dir, logger=None, early_stopping_patience=5, checkpoint_path='clip_hba_model_cv.pth', dora_parameters_path='./dora_params', model_embedding_path='./model_embeddings', model_rdm_path='./model_rdms'):
    ...
    for epoch in range(epochs):
        ...
        if epoch == 0 or epoch in curriculum_save_epochs:
            sorted_model_embeddings = update_difficulties(model, difficulty_loader, device, difficulty_criterion)
            # Save the sorted model embeddings df to a csv file
            log(f"Saving sorted model embeddings df to csv file at epoch {epoch+1}")
...
def run_behavioral_traning(config):
    ...
    for i in range(5):
        ...
        logger.info(f"Train dataset sample {i}: {sample}")
    ...
    difficulty_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=False)

    # Print first 5 samples of the train loader
    for i in range(5):
        sample = train_loader.dataset[i]
        logger.info(f"Train data loader sample {i}: {sample}")
    ...
    # Train model
    train_model(model, train_loader, test_loader, inference_loader, difficulty_loader, device, optimizer, 
                config['criterion'], 
                config['epochs'], 
                ...
                model_embedding_path=config['model_embedding_path'],
                model_rdm_path=config['model_rdm_path'])