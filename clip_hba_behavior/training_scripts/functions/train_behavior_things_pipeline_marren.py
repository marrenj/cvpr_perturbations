...
def train_model(model, train_loader, test_loader, inference_loader, device, optimizer, criterion, epochs, difficulty_criterion,training_res_path, early_stopping_patience=5, checkpoint_path='clip_hba_model_cv.pth', dora_parameters_path='./dora_params', model_embedding_path='./model_embeddings', model_rdm_path='./model_rdms'):
    model.train()
    best_test_loss = float('inf')
    epochs_no_improve = 0
    loss_data = []  # To store loss data for plotting
    behavioral_rsa_data = [] # To store behavioral RSA data for plotting
    ...
    for epoch in range(epochs):
        ...
        rho, p_value = behavioral_RSA(model, inference_loader, device)
