...
def update_difficulties(model, train_loader, device, difficulty_criterion):
        ...
        # Sort the embeddings DataFrame by loss value (DESCENDING for anti-baby-step - hardest first)
...
def load_previous_training_results(training_results_path):
    """Load previous training results from CSV file"""
    if os.path.exists(training_results_path):
        df = pd.read_csv(training_results_path)
        print(f"Loaded previous training results with {len(df)} epochs")
        return df
    else:
...
def train_model_resume(model, train_loader, test_loader, inference_loader, device, optimizer, criterion, epochs, training_res_path, difficulty_criterion, batch_size, csv_file, img_dir, early_stopping_patience=5, start_rate=.3, grow_rate=.05, grow_interval=5, checkpoint_path='clip_hba_model_cv.pth', dora_parameters_path='./dora_params', model_embedding_path='./model_embeddings', curriculum_subset_path='./curriculum_subsets', model_rdm_path='./model_rdms', random_target_epoch=None, random_target_seed=42, start_epoch=0, resume_checkpoint_path=None, resume_training_results_path=None):
    model.train()
    best_test_loss = float('inf')
    epochs_no_improve = 0

    # Load previous training results if resuming
    previous_results = None
    if resume_training_results_path and os.path.exists(resume_training_results_path):
        previous_results = load_previous_training_results(resume_training_results_path)
        # Find the best test loss from previous results
        if previous_results is not None and len(previous_results) > 0:
            best_test_loss = previous_results['test_loss'].min()
            print(f"Resuming with best test loss: {best_test_loss:.4f}")

    # initial evaluation
    print("*********************************")
    print("Evaluating initial model")
    initial_test_loss = evaluate_model(model, test_loader, device, criterion)
    print(f"Initial Validation Loss: {initial_test_loss:.4f}")
    print("*********************************\n")
    ...
    # If resuming, copy previous results to new file
    if previous_results is not None:
        # Copy previous results to new training results file
        previous_results.to_csv(training_res_path, index=False)
        print(f"Copied previous results to {training_res_path}")
        
        # If we're starting from epoch 46, we need to update the curriculum state
        if start_epoch > 0:
            # Calculate the current curriculum state based on start_epoch
            current_portion = start_rate + grow_rate * (start_epoch // grow_interval)
            current_portion = min(1.0, current_portion)
            print(f"Resuming with curriculum portion: {current_portion}")
    else:
    ...
    # Calculate difficulties for the first epoch or resume epoch
    if start_epoch == 0 or start_epoch % 5 == 0:
        print(f"Re-calculating difficulties at epoch {start_epoch + 1}")
        ...
        # For resume, we'll need to recalculate difficulties
        print(f"Re-calculating difficulties for resume at epoch {start_epoch + 1}")
        sorted_model_embeddings = update_difficulties(model, train_loader, device, difficulty_criterion)
        curriculum_dataset = ThingsDataset(csv_file, img_dir, filter_indices=sorted_model_embeddings['original_index'].values)
        curriculum_data_loader = DataLoader(curriculum_dataset, batch_size, shuffle=False)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Update difficulties every 5 epochs instead of every epoch
        if epoch == start_epoch or epoch % 5 == 0:
            ...
            # Reuse the curriculum dataset from the last difficulty update
            print(f"Reusing curriculum dataset from epoch {((epoch // 5) * 5) + 1}")
            curriculum_dataset = ThingsDataset(csv_file, img_dir, filter_indices=sorted_model_embeddings['original_index'].values)
            curriculum_data_loader = DataLoader(curriculum_dataset, batch_size, shuffle=False)
        ...
        if epoch == start_epoch or epoch in curriculum_save_epochs:
        ...
        if current_portion != 1.0: # if the data portion is not 100%
            epochs_no_improve = 0 
...
def run_behavioral_traning_resume(config):
    """
    Run behavioral training with the given configuration, resuming from a checkpoint.
    ...
    if config.get('resume_checkpoint_path') and os.path.exists(config['resume_checkpoint_path']):
        print(f"Loading checkpoint from {config['resume_checkpoint_path']}")
    ...
    print(f"Resume checkpoint: {config.get('resume_checkpoint_path', 'None')}")
    print(f"Resume training results: {config.get('resume_training_results_path', 'None')}")
    print("*** END RESUME CONFIGURATION ***\n")

    # Train model
    train_model_resume(model, train_loader, test_loader, inference_loader, device, optimizer, 
                config['criterion'], 
                config['epochs'], 
                ...
                config['start_epoch'],
                config.get('resume_checkpoint_path'),
                config.get('resume_training_results_path'))
