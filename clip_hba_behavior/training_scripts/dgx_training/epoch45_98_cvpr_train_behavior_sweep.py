...
def main():
    ...
    config = {
        'csv_file': '/data/p_dsi/dhungs1/Data/spose_embedding66d_rescaled_1806train.csv', # csv data annotations of the training stimuli with the corresponding target embeddings
        'img_dir': '/data/p_dsi/dhungs1/Data/Things1854', # path to the image directory
        'inference_csv_file': '/data/p_dsi/dhungs1/Data/spose_embedding66d_rescaled_48val_reordered.csv', # csv data annotations of the inference stimuli with the corresponding target embeddings
        'RDM48_triplet_dir': '/data/p_dsi/dhungs1/Data/RDM48_triplet.mat', # location of the reference behavioral RDM
        ...
        'baseline_dora_directory': '/data/p_dsi/dhungs1/baseline_runs/clip_hba_behavior_seed1/training_artifacts/dora_params/dora_params_seed1', # location of the DoRA parameters for the baseline training run
        'baseline_random_state_path': '/data/p_dsi/dhungs1/baseline_runs/clip_hba_behavior_seed1/training_artifacts/random_states/random_states_seed1', # location of the random states for the baseline training run
        'baseline_split_indices_path': '/data/p_dsi/dhungs1/baseline_runs/clip_hba_behavior_seed1/training_artifacts/random_states/random_states_seed1/dataset_split_indices.pth', # location of the train/test split indices from baseline training
        ...
        'output_base_directory': '/data/p_dsi/dhungs1/image_noise_single_epoch_perturbation_sweeps/perturb_sweep_baselineseed1_perturbseed42_{timestamp}', # base directory for saving the training results and artifacts
    }
    ...
    main_logger.info("="*80)
    main_logger.info("STARTING MAIN TRAINING LOOP - IMAGE NOISE PERTURBATION")
    ...
    # Define your training order here
    training_order = [45, 98]
    
    main_logger.info(f"Training order: {training_order}")
    main_logger.info(f"Total epochs to train: {len(training_order)}")
    main_logger.info("")

    for idx, training_run in enumerate(training_order, 1):
        main_logger.info("-"*80)
