...
def main():
    ...
    config = {
        ...
        'checkpoint_path': './models/cliphba_behavior.pth', # path to save the trained model weights
    ...
    if config['use_curriculum_learning']:
        if config['curriculum_algorithm'] == 'baby_step':
            from functions.train_behavior_things_baby_step_pipeline_random_targets import run_behavioral_traning
        elif config['curriculum_algorithm'] == 'anti_baby_step':
            from functions.train_behavior_things_anti_baby_step_pipeline_random_targets import run_behavioral_traning
    else:
        from functions.train_behavior_things_baseline_pipeline_marren import run_behavioral_traning
